...
public class KeyHandle implements IInputProvider {

    public Int2CharOpenHashMap mappedKeys = new Int2CharOpenHashMap();
    public Int2ObjectOpenHashMap<String> namedKeys = new Int2ObjectOpenHashMap<>();
    public Int2CharOpenHashMap shiftMappedKeys = new Int2CharOpenHashMap();
    public boolean[] keyPressed = new boolean[350];

    public ConcurrentLinkedQueue<CharPress> charPresses = new ConcurrentLinkedQueue<>();
    public ConcurrentLinkedQueue<Tuple<KeyPress, Integer>> keyPresses = new ConcurrentLinkedQueue<>();

    public void register(KeyPress keyPress, int id) {
    ...
    public void register(CharPress charPress) {
        charPresses.add(charPress);
    }

    public void remove(KeyPress keyPress) {
    ...
    public void remove(CharPress charPress) {
        charPresses.remove(charPress);
    }

    private void onPress(int id) {
        char key = getChar(id);
    ...
    private void setCallback(GLInitEvent glInitEvent) {
        setCallback(glInitEvent.window);
    }

    private void putKeyAndKey(int id, char key) {
        mappedKeys.put(id,key);
        namedKeys.put(id,key + "");
    }

    private void putName(int id, String name) {
        namedKeys.put(id,name);
    }

    public KeyHandle(long Window) {
